...
public class HotelController {
    ...
    public Dto<ItripAreaDicVO> querytradearea(@PathVariable Integer type) {
        ...
        return DtoUtil.returnSuccess("查询热门城市商圈",itripAreaDicVOs);
    }
    ...
    public Dto<ItripSearchFacilitiesHotelVO> queryhotelfacilities(@PathVariable Integer cityId) {
        ItripSearchFacilitiesHotelVO itripSearchFacilitiesHotelVOS = new ItripSearchFacilitiesHotelVO();
        ItripHotel itripHotelList=null;
        try {
            logger.debug("进来了》》》》》");
            System.out.println("查询酒店设施》》》》》》》》》》》》》》");
            itripHotelList=itripHotelService.getItipHotelBycityId(cityId);
            itripSearchFacilitiesHotelVOS.setFacilities(itripHotelList.getFacilities());
        } catch (Exception e) {
            e.printStackTrace();
            return   DtoUtil.returnFail("系统异常", "10202");
        }
        return DtoUtil.returnSuccess("获取信息成功",itripSearchFacilitiesHotelVOS.getFacilities());
    }
    ...
    public Dto<ItripSearchPolicyHotelVO> queryhotelpolicy(@PathVariable Integer cityId) {
       ItripSearchPolicyHotelVO itripSearchPolicyHotelVO = new ItripSearchPolicyHotelVO();
        ...
        try {
            itripHotelList=itripHotelService.getItipHotelPolicyBycityId(cityId);
            itripSearchPolicyHotelVO.setHotelPolicy(itripHotelList.getHotelPolicy());
        } catch (Exception e) {
            DtoUtil.returnFail("系统异常", "10202");
            e.printStackTrace();
        }
        return DtoUtil.returnSuccess("获取信息成功",itripSearchPolicyHotelVO.getHotelPolicy());
    }
     ...
     * @return
     */
    @RequestMapping(value = "/queryhoteldetails/{type}", produces = "application/json", method = RequestMethod.GET)
    @ResponseBody
    public Dto<ItripSearchDetailsHotelVO> queryhoteldetails (@PathVariable Integer type) {
        ...
        try {
            if (EmptyUtils.isNotEmpty(type)) {
                Map param = new HashMap();
                param.put("parentId",16);
                itripSearchDetailsHotelVOS = itripLabelDicService.getItripLabelDicListByMap(param);
                if (EmptyUtils.isNotEmpty(itripSearchDetailsHotelVOS)) {
                    itripHotelList = itripHotelService.getItipHotelById(type);
                        ItripSearchDetailsHotelVO vo = new ItripSearchDetailsHotelVO();
                        vo.setName("酒店介绍");
                        //vo.setDescription(itripHotelList.getDetails());
